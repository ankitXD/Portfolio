...
const generatePassword = (len, upper, lower, nums, syms) => {
  let charset = "";
  if (upper) charset += UPPER;
  if (lower) charset += LOWER;
  if (nums) charset += DIGITS;
  if (syms) charset += SYMBOLS;
  ...
  const guarantees = [];
  if (upper) guarantees.push(UPPER);
  if (lower) guarantees.push(LOWER);
  if (nums) guarantees.push(DIGITS);
  if (syms) guarantees.push(SYMBOLS);

  const arr = pwd.split("");
  const rand = new Uint32Array(guarantees.length * 2);
  crypto.getRandomValues(rand);
  guarantees.forEach((set, gi) => {
    const pos = rand[gi] % arr.length;
    arr[pos] = set[rand[gi + guarantees.length] % set.length];
  });

  return arr.join("");
};

/* ───── toggle (module-scope to preserve state across re-renders) ───── */
...
const PasswordGenerator = () => {
  ...
  const regenerate = (overrides = {}) => {
    ...
    const upper = overrides.uppercase ?? uppercase;
    const lower = overrides.lowercase ?? lowercase;
    const nums = overrides.numbers ?? numbers;
    const syms = overrides.symbols ?? symbols;
    const pwd = generatePassword(len, upper, lower, nums, syms);
    setPassword(pwd);
    if (pwd)
      setHistory((prev) =>
        [{ id: ++historyIdRef.current, pwd }, ...prev].slice(0, 5),
      );
    setCopied(false);
  };
        ...
        <div className="flex flex-col items-center justify-center min-h-screen px-4 py-16 sm:px-6">
          ...
          >
            ...
            <p className="mt-3 text-neutral-500 dark:text-neutral-400 max-w-md mx-auto text-sm sm:text-base">
              Generate cryptographically secure passwords tailored to your
              needs.
            </p>
          </m.div>
            ...
            <div className="rounded-2xl border border-neutral-200/80 dark:border-neutral-700/60 bg-white/70 dark:bg-neutral-900/70 backdrop-blur-xl shadow-xl shadow-violet-500/5 dark:shadow-black/20 overflow-hidden">
              ...
              <div className="p-5 sm:p-6 border-b border-neutral-200/60 dark:border-neutral-700/40">
                ...
                <div className="mt-4 flex items-center gap-3">
                  <StrengthIcon
                    className={`w-4 h-4 shrink-0 ${strength.text}`}
                  />
                    ...
                    <div className="divide-y divide-neutral-100 dark:divide-neutral-800">
                      {history.slice(1).map((entry, i) => (
                        ...
                        >
                          ...
                          <button
                            onClick={async () => {
                              await navigator.clipboard.writeText(entry.pwd);
                            }}
